...
export async function GET() {
  ...
  const ratings = Object.fromEntries(
    (data ?? []).map((row) => [row.target_user_id as string, row.rating as number])
  );
...
export async function POST(req: Request) {
  ...
  const { data: targetProfile, error: targetError } = await auth.supabase
    .from("profiles")
    .select("user_id")
